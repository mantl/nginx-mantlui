...
Elm.Attributes.make = function (_elm) {
   ...
   if (_elm.Attributes.values)
   return _elm.Attributes.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var classes = function (cs) {
      return $Html$Attributes.classList($List.map(function (m) {
         return {ctor: "_Tuple2"
                ,_0: m
                ,_1: true};
      })(cs));
   };
   _elm.Attributes.values = {_op: _op
                            ,classes: classes};
   return _elm.Attributes.values;
};
   ...
   model) {
      return function () {
         ...
         {case "HealthAction":
            ...
            case "ServicesAction":
            return function () {
                 var $ = A2($Services.update,
                 action._0,
                 model.services),
                 services = $._0,
                 fx = $._1;
                 return {ctor: "_Tuple2"
                        ,_0: _U.replace([["services"
                                         ,services]],
                        model)
                        ,_1: A2($Effects.map,
                        ServicesAction,
            ...
            case "VersionAction":
            return function () {
                 var $ = A2($Version.update,
                 action._0,
                 model.version),
                 version = $._0,
                 fx = $._1;
                 return {ctor: "_Tuple2"
                        ,_0: _U.replace([["version"
                                         ,version]],
                        model)
                        ,_1: A2($Effects.map,
                        VersionAction,
...
Elm.Route.make = function (_elm) {
   ...
   var locFor = function (path) {
      return function () {
         var segments = $List.filter(function (seg) {
            return !_U.eq(seg,
            "") && !_U.eq(seg,"#");
         })($String.split("/")(path));
         return function () {
...
Elm.Services.make = function (_elm) {
   ...
   if (_elm.Services.values)
   return _elm.Services.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   service) {
      ...
      _L.fromArray([$Attributes.classes(_L.fromArray(["col-sm-3"
                                                     ,"service"]))]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Attributes.classes(_L.fromArray(["card"
                                                     ,"card-block"]))]),
      _L.fromArray([A2($Html.div,
                   ...
                   _L.fromArray([$Html.text(service.name)]))
                   ,A2($Html.a,
                   _L.fromArray([$Attributes.classes(_L.fromArray(["btn"
                                                                  ,"btn-block"
                                                                  ,"btn-primary"]))
               ...
               return A2($Html.div,
                 ...
                 _L.fromArray([A2($Html.div,
                              ...
                              _L.fromArray([$Attributes.classes(_L.fromArray(["btn"
                                           ...
                                           address,
                                           LoadServices)]),
                              _L.fromArray([$Html.text("Reload Services")]))]))]))
                              ,A2($Html.div,
                              _L.fromArray([$Attributes.classes(_L.fromArray(["row"
                                                                             ,"services"]))]),
                              $List.map(function (s) {
   ...
   "path",
   $Json$Decode.string));
   var loadServices = $Effects.task($Task.map(NewServices)($Task.toMaybe(A2($Http.get,
...
Elm.StartApp.make = function (_elm) {
   ...
   var start = function (config) {
      return function () {
         var updateStep = F2(function (_v0,
         _v1) {
            return function () {
               switch (_v1.ctor)
               {case "_Tuple2":
                  return function () {
                       ...
                       {case "_Tuple2":
                          return function () {
                               ...
                               return {ctor: "_Tuple2"
                                      ,_0: newModel
                                      ,_1: $Effects.batch(_L.fromArray([_v1._1
                                                                       ,additionalEffects]))};
                            }();}
                       _U.badCase($moduleName,
         ...
         A2($List.map,
         $Signal.map(function ($) {
            return singleton(F2(function (v0,
            v1) {
               return {ctor: "_Tuple2"
                      ,_0: v0
                      ,_1: v1};
...
Elm.Version.make = function (_elm) {
   ...
   if (_elm.Version.values)
   return _elm.Version.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var version = F2(function (address,
   model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("version")]),
      _L.fromArray([$Html.text(A2($Maybe.withDefault,
      "No Version",
      model.current))]));
   });
   var view = version;
   var notification = F2(function (address,
   model) {
      ...
      _L.fromArray([$Html$Attributes.$class("container")]),
      _L.fromArray([A2($Html.p,
      _L.fromArray([$Html$Attributes.$class("nav-item")]),
      _L.fromArray([$Html.text("New version available. Please reload!")]))]))])) : A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([]));
   });
   var update = F2(function (action,
            ...
            switch (action._0.ctor)
              {case "Just":
                 return function () {
                      ...
                      {case "Just":
                         return _U.eq(_v3._0,
                           action._0._0) ? {ctor: "_Tuple2"
                                           ,_0: model
                                           ,_1: $Effects.none} : {ctor: "_Tuple2"
                                                                 ,_0: _U.replace([["current"
              ...
              break;}
         _U.badCase($moduleName,
         "between lines 28 and 46");
      }();
   });
   var NewVersion = function (a) {
...
Elm.WebUI.make = function (_elm) {
   ...
   if (_elm.WebUI.values)
   return _elm.WebUI.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Task = Elm.Task.make(_elm),
   $Time = Elm.Time.make(_elm);
   var refresh = $Signal.map(function (_v0) {
      return function () {
         return $Mantl.Refresh;
      }();
   })($Time.every(10 * $Time.second));
   var hash = A2($Signal.map,
   function ($) {
      return $Mantl.RouteAction($Route.PathChange($));
   },
   $History.hash);
   var app = $StartApp.start({_: {}
                             ,init: $Mantl.init
                             ,inits: _L.fromArray([hash])
                             ,inputs: _L.fromArray([refresh])
                             ,update: $Mantl.update
                             ,view: $Mantl.view});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",
   app.tasks);
   _elm.WebUI.values = {_op: _op
                       ,app: app
                       ,main: main
                       ,hash: hash
                       ,refresh: refresh};
